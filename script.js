...
function rollDice() {
  ...
  if (player.position > 25) player.position = 25; // ถึง 25 ก็หยุด

  const before = player.position;
  player.position = applySnakesLadders(player.position);
  ...
  if (player.position >= 25 && !player.finished) {
...
db.ref("winner").on("value", snap => {
  ...
  if (winner && !player.finished) {
    ...
    player.finished = true;
    // ปิดปุ่มทอย (ถ้ามีปุ่มเฉพาะ)
    const askBtn = document.getElementById("askBtn") || document.querySelector("button");
